...
1. **Rabbit terrain/geometry pipeline + static 3D viewer** (`scripts/loadGeometry.py` → repo root) — generate `.glb` files from terrain/building/neighborhood source data and view them in the browser with Three.js. No backend required. The viewer (`index.html`, `rabbitViewer.js`) and its generated output (`*.glb`, `scenarios/`, `scenarios.json`, `polylines/`, `polylines.json`) live directly in the repo root.
2. **Flask backend API** (`backend/`) — a separate file-upload/processing API. It was originally built for a React/Vite frontend that has since been removed; nothing in this repo currently consumes it.

They don't depend on each other. If you just want to view a site's terrain, you only need Part 1.

---

## Part 1: Preparing and viewing the 3D scene

### 1.1 Generating the `.glb` files
...
The source file for the neighborhood is generated from the geojson files that can be found in the folder: `Rabbit Configurator - Documents\02 Input\_BUND Swiss Buildings 3.0\`
...
**Where the source files come from:** each site's full project export from the Rabbit Configurator/report pipeline (e.g. a folder named like `WeinfeldenCH440666779312_CH458366770673`) contains everything used for report generation — `.pkl`, `.html` reports, `.tif` rasters, `.dxf` exports, floorplan `.png`s, etc. — not just the terrain/building files `loadGeometry.py` needs. The `--site-dir` folder you point the script at (e.g. `Weinfelden_CH440666779312_CH458366770673_viewerInputs/`) should contain only the curated subset described in the table below, pulled out of that full project export. These project-export folders are large (100MB+), so they're generally kept out of this repo — store them wherever you keep Rabbit Real Estate project data locally, and point `--site-dir` at your own curated copy.

### 1.2 Quick start: `--site-dir` (recommended)

Put all of a site's source files in one folder (e.g. `sample-data/Weinfelden_CH440666779312_CH458366770673_viewerInputs/`) and point `--site-dir` at it — the script auto-discovers everything by filename, no need to spell out every path:

```
python scripts/loadGeometry.py --site-dir sample-data\Weinfelden_CH440666779312_CH458366770673_viewerInputs --geojson-dir "C:\Users\panz\Rabbit Real Estate\Rabbit Configurator - Documents\02 Input\_BUND Swiss Buildings 3.0" --swiss-buildings-radius-km 0.5
```
...
- `--swiss-buildings-output` defaults to `neighborhood.glb` in the repo root (what the viewer actually loads).
...
### 1.3 Viewing the scene in a browser

`rabbitViewer.js` loads the generated `.glb` files, `scenarios.json`, and `polylines.json` via `fetch()`. **Don't open `index.html` directly as a `file://` URL** — browsers block `fetch()` against local files, so the page will load with a blank scene and console errors like "Failed to fetch". Serve the repo root over local HTTP instead:
...
Note: since the viewer files live at the repo root, this also exposes the rest of the repo (source data, scripts, backend code) over that local server — harmless for local dev, just don't do this on a shared/public network.
...
If `scenarios.json` exists, the viewer builds a tab bar (top-left) from it and loads the first scenario's modified terrain + buildings by default. Clicking a tab disposes the previous scenario's meshes and loads the new one — only one scenario's terrain/buildings is in memory at a time. If `scenarios.json` is missing (404), the viewer falls back to loading fixed `modifiedTerrain.glb` / `buildings.glb` files instead — useful for a single-scenario site with no tabs.
...
## Part 2: Backend API setup (`backend/`)

This was the backend API server for a React/Vite frontend that has since been removed from this repo — it's a separate track from the static viewer above, isn't required for it, and currently has no consumer.
...
- `http://localhost:5173` (Vite dev server — no longer present in this repo, kept here in case a frontend is reintroduced)
